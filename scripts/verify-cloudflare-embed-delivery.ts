...
import { createCloudflareEmbedDeliveryApplication } from '../services/embed-delivery-service/src/infrastructure/create-cloudflare-embed-delivery-application.ts'
import type { AssetsBinding, D1Database, D1PreparedStatement } from '../server/lib/cloudflare/bindings.ts'
...
class FakeD1Database implements D1Database {
  ...
  async first<T>(query: string, values: unknown[]): Promise<T | null> {
    ...
    if (query.includes('FROM tenant_users WHERE lower(email) = lower(?)')) return (Array.from(this.tenantUsers.values()).find((item) => item.email === String(values[0]).toLowerCase()) as T) || null
    return null
  }
...
class FakeAssetsBinding implements AssetsBinding {
  async fetch(input: Request | string): Promise<Response> {
    const url = typeof input === 'string' ? input : input.url
    if (url.includes('customer-bot.js')) {
      return new Response('window.CustomerBot = { init() {} }', {
        status: 200,
        headers: {
...
async function main() {
  ...
  await tenantApp.createTenant({
    id: 'tenant-embed',
    name: 'Tenant Embed',
    contactEmail: 'tenant-embed@example.com'
  })

  const app = createCloudflareEmbedDeliveryApplication(bindings)
  const config = await app.runtimeConfig({ tenantId: 'tenant-embed' })
  assert.equal(config.tenantId, 'tenant-embed')

  const script = await app.widgetScript()
  assert.equal(script.code.includes('CustomerBot'), true)
  assert.equal(script.contentType.includes('javascript'), true)

  console.log('cloudflare embed delivery verified')
}
